...
class CNN:
    ...
    def fit_chn(self, X_train, y_train, X_test=None, y_test=None):
        ...
        #cnn = cL.Classifier(Net)
        cnn = cL.Classifier(Net, lossfun=cF.hinge)

        #cnn = Net(nb_filter, nb_conv, nb_pool, n_out)
        self.model_2d = None
        #cnn = Net()
        ...
        for i in xrange(nb_epoch):
            ...
            for j in xrange(0, a_tr, batch_size):
                ...
                #Y_p = cnn(X)
                #loss = cF.softmax_cross_entropy(Y_p, Y)
            ...
            print('train iteration', i, 'precision:', prc, 'recall:', rec, 'f1:', f1, 'data_size:', X_train.shape)
            #continue
            ...
            rec = metrics.recall_score(y_test, y_test_p)
            f1 = metrics.f1_score(y_test, y_test_p)
            print('test iteration', i, 'precision:', prc, 'recall:', rec, 'f1:', f1, 'data_size:', X_test.shape)
